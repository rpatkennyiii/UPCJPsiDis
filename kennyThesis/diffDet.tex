\chapter{The CMS Detector}	
  ...
  This is the scale where electroweak symmetry breaking is believed to occur
  	\cite{CmsPTdrv2}.
  While this means that the search for the standard model \DIFdelbegin \DIFdel{Higs is }\DIFdelend \DIFaddbegin \DIFadd{Higgs boson was }\DIFaddend the 
  ...
  \DIFdelbegin \DIFdel{This consideration leads inevitably to pile up, where multiple collisions 
	occurs at a single bunch crossing.
  At peak luminosity the LHC is expected to produce on average 20 hard 
	proton-proton (pp) collisions per bunch crossing \mbox{%DIFAUXCMD
\cite{tCmsE}
}%DIFAUXCMD
. 
  These particle physics considerations of high multiplicity due to pileup and the need for a general purpose detector make CMSserendipitously well suited
	for heavy ion physics. 
}\DIFdelend \DIFaddbegin \DIFadd{While the difference is scale, 125 GeV for the mass of the Higgs bosson as 
    compared to 3.1 GeV for the mass of the }\JPsi\DIFadd{, create difficulties which 
    will be discussed below, these two considerations created the opportunity 
    to explore UPC }\JPsi\DIFadd{.
  ...
  Muon capabilities developed for this process can be used to study }\JPsi\DIFadd{, 
  ...
  By exploiting the versatility of the trigger and CMS's muon systems it
    is possible to explore process like UPC }\JPsi \DIFadd{production, which push to the
  ...
  \DIFaddbegin \section{\DIFadd{Tracker}}
    ...
    \DIFdelbegin \DIFdel{Pseudorapidity is defined as $\eta\equiv-\ln(\tan(\theta/2))$, where $\theta$ is 
	the polar angle, and $\phi$ is the azimuthal angle with respect to the 
	beam axis.
}\DIFdelend At the center of the tracker are three rings of silicon pixels around the beam 
    ...
    \DIFaddbegin \begin{figure}[!Hhbt]
      ...
      \caption{\DIFaddFL{Layout of the silicon tracker with the pixels closest to the 
    ...
    \DIFadd{The amount of material present in the tracker is substantial enough to
    ...
    As opposed to the deflection angle set by the strength of the magnetic 
      ...
      the detector.
    For UPC }\JPsi\DIFadd{~this is the primary factor contributing to the resolution of
    ...
    \begin{figure}[!Hhbt]
      ...
      \caption{\DIFaddFL{Material in the tracker broken down by sub-detector(left) and
        category (right).}}
      \label{fig:matBudge}
    \end{figure}

  \section{\DIFadd{ECAL}}
    \DIFaddend The next detector beyond the tracker is ECAL.
    ...
    The crystals of the barrel have a depth of 230 mm corresponding to 25.8 
    	\DIFdelbegin \DIFdel{radiation lengths (}\DIFdelend $X_{0}$\DIFdelbegin \DIFdel{).
...
The }\DIFdelend \DIFaddbegin \DIFadd{.
    The }\DIFaddend endcaps (EE) cover the psuedorapitity region $1.479 < |\eta| < 3$.
  ...
  \DIFaddbegin \section{\DIFadd{HCAL}}
  ...
  \DIFdelbegin \DIFdel{Beyond HF there are two more detectors in the forward region.
CASTOR covers the range 5.2 $< \eta <$ 6.6 on the positive side of the beam. 
The }\DIFdelend \DIFaddbegin \section{\DIFadd{ZDC}} 
    \DIFadd{Beyond HF, the }\DIFaddend Zero Degree Calorimeters(ZDC) \DIFaddbegin \DIFadd{covers the very forward 
      rapidity region.
    ZDC }\DIFaddend sit between the beam pipes on either side of the interaction point 
    ...
    \DIFdelbegin \DIFdel{CASTOR extends the total coverage of the CMS as whole giving more access to 
	low-x physics \mbox{%DIFAUXCMD
\cite{tCmsE}
}%DIFAUXCMD
.
}\DIFdelend 
  ...
  \DIFaddbegin \section{\DIFadd{Muons}}
    \DIFaddend The muon system resides just outside of the superconducting magnet.
    ...
    \DIFdelbegin \DIFdel{Ultimately the muon system is most useful for triggering on muons \mbox{%DIFAUXCMD
\cite{tCmsE}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Each of the gaseous detectors function in the same basic way.
    ...
    The combination of the DTs and RPCs in the barrel and the CSCs and RPCs 
      ...
      data reconstruction}\DIFaddend . 

    \DIFdelbegin \DIFdel{The heavy ion community is making use of the capabilities of CMS in a myriad of ways. 
	...
	known as the quark gluon plasma (QGP)}\DIFdelend \DIFaddbegin \begin{figure}[!Hhbt]
      \centering
      \includegraphics[width=.65\textwidth]{muonSys}
      \caption{\DIFaddFL{ The CMS muon system showing the four DT stations in 
        the barrel (MB1-MB4), the four CSC stations in the endcap (ME1-ME4), 
        and the RPC stations.}}
      \label{fig:muonSys}
    \end{figure}

    \DIFadd{As seen in Fig.~\ref{fig:muonSys}, the DTs reside only in the barrel, 
   ...
   Super layers SL $\Phi_{1}$ and SL $\Phi_{2}$ measure $(r,\phi)$, whereas 
    SL $\Theta$ measures $z$}\DIFaddend .
\DIFdelbegin \DIFdel{The tracker has been utilized for to study charged particle multiplicities, }\DIFdelend \DIFaddbegin 

    \begin{figure*}[!Hhbt]
      \centering
      \includegraphics[width=.65\textwidth]{dtStruct} \\
      \includegraphics[width=.65\textwidth]{dtCell}
      \caption{\DIFaddFL{Schematic of the DT chambers and an individual DT cell.}}
      \label{fig:dtSchem}
    \end{figure*}

    \DIFadd{The RPC complement the DTs in the barrel and the CSCs in the endcap 
    ...
    Six layers of RPC chambers are embedded in the barrel iron yoke, two
      ...
      to $r$ segment in the DTs MB1 }\DIFaddend and \DIFaddbegin \DIFadd{RB2 with MB2 etc. 
    The redundancy in the first two stations allows the trigger algorithm to 
      ...
      }\pt \DIFadd{particles, which may stop inside the iron yoke. 
    ...
    \begin{figure}[!Hhbt]
      ...
      \caption{\DIFaddFL{Schematic of a RPC cell.}}
      \label{fig:rpcSchem}
    \end{figure}

    \DIFadd{The CMS Endcap Muon system will consist of 468 CSC arranged in groups as
    ...
    Strips are on cathode panels and run lengthwise at constant $\Delta\phi$ 
      width. 
    The largest chambers, ME2/2 }\DIFaddend and \DIFdelbegin \DIFdel{elliptical flow, two probes of }\DIFdelend \DIFaddbegin \DIFadd{ME3/2, are about 3.4x1.5 m$^{2}$ in size. 
    ...
    \begin{figure}[!Hhbt]
      \centering
      \DIFaddFL{$ \begin{array}{cc}
        \includegraphics[width=.35\textwidth]{cscStruct} &
        \includegraphics[width=.65\textwidth]{cscCell}
      \end{array} $
      }\caption{\DIFaddFL{Schematic of the CSC chambers and an individual CSC cell.}}
      \label{fig:cscSchem}
    \end{figure}

    \DIFadd{The primary function of the muon systems are to allow for triggering on and
    ...
    Fig.~\ref{fig:muonRes} shows the resolution of reconstructed muons with 
      }\DIFaddend the \DIFdelbegin \DIFdel{thermal expansion of the QGP. 
	...
	collisions.
    %DIF <   \section{CMS general}
%DIF <   \section{Muons}
%DIF <   \section{HCal}
%DIF <   \section{ZDC}
  }\DIFdelend \DIFaddbegin \DIFadd{tracker only is only improved upon for muons with momenta above 
      100 GeV. 
    For UPC }\JPsi\DIFadd{~events where muons momenta are between 1-2 GeV, the muon
    ...
    \begin{figure}[!Hhbt]
      ...
      \caption{\DIFaddFL{ The momentum resolution of the muon system using only the 
    ...
    \DIFadd{The low-momentum nature of UPC physics creates complications due to the 
    ...
    About 3 GeV of momentum is needed to reach the first layers of the muon 
      system.
    In the rest frame of the }\JPsi\DIFadd{, the }\JPsi \DIFadd{equally shares its rest mass with 
      its decay products creating 2 muons with momenta of about 1.5 GeV.
    For these daughter muons to reach the muon system, their parent }\JPsi \DIFadd{must
    ...
    \begin{figure}[!Hhbt]
      ...
      \caption{\DIFaddFL{The amount of material in CMS as a function of $\eta$ in 
        number of interaction lengths.}}
      \label{fig:matThick}
    \end{figure}

  \DIFaddend \section{Trigger}
    The CMS trigger is two \DIFdelbegin \DIFdel{teired}\DIFdelend \DIFaddbegin \DIFadd{tiered}\DIFaddend . 
    The L1 trigger is the lower level \DIFdelbegin \DIFdel{hardward }\DIFdelend \DIFaddbegin \DIFadd{hardware }\DIFaddend based system. 
    ...
    If an event passes the L1 trigger, the data from all the sub-detectors
      ...
      }\DIFdelend including the Tracker are sent to the HLT computing farm. 
