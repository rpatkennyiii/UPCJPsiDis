\chapter{Photo-nuclear interactions \label{ch:photoNuc}}
    ...
    Other initial state probes such as peripheral nuclear collisions and 
      ...
      \DIFdelbegin \DIFdel{which effects }\DIFdelend \DIFaddbegin \DIFadd{effects that }\DIFaddend come from the initial state.
    \DIFdelbegin \DIFdel{In UPC events the nuclei do not collide}\DIFdelend \DIFaddbegin \DIFadd{Hadronic interactions do no occur in ultra-peripheral collisions (UPC)}\DIFaddend , 
    ...
    There are other UPC models available~\cite{Goncalves:2011vf,Cisek:2012yt,Lappi:2013am,Hufner:1996jw}, 
      but this thesis focuses on \DIFdelbegin \DIFdel{STARlight because it is used as the Monte 
      Carlo generator throughout, and the ABand LTA models 
      show the best agreement with previous measurements by the ALICE 
      Collaboration~\mbox{%DIFAUXCMD
\cite{Abelev:2012ba,Abbas:2013oua}
}%DIFAUXCMD
. 
    They have provided important constrains to modelsof the nuclear gluon 
      density~\mbox{%DIFAUXCMD
\cite{lta2013.05}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the STARlight, the AB, and the LTA models}\DIFaddend .
    The analysis presented in Chapter~\ref{ch:analysis} adds to the existing 
      data in a new kinematic region.
    In this Chapter\DIFaddbegin \DIFadd{, }\DIFaddend the theoretical framework for photon-nuclear
  ...
  \section{Weizs\"{a}cker-Williams approximation \label{sec:wwAprox}}
    ...
    The approximation is semi-classical\DIFdelbegin \DIFdel{and combines both classical and quantum 
      elements}\DIFdelend . 
    ...
    \begin{figure}[!Hhbt]
      ...
      \end{center}
      \caption{ \label{fig:boost} The electromagnetic field boosted \DIFaddbeginFL \DIFaddFL{(right) }\DIFaddendFL and 
        at rest \DIFaddbeginFL \DIFaddFL{(left) \mbox{%DIFAUXCMD
\cite{WWJackson}
}%DIFAUXCMD
}\DIFaddendFL .}
    \end{figure}
  ...
  \section{\label{sec:vdmTheory}The STARlight model}
    ...
    photon flux, uses the vector meson dominance\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Bauer:1977iq}
}%DIFAUXCMD
}\DIFaddend fit to the proton-electron data, 
    ...
    The proton-electron scattering data \DIFdelbegin \DIFdel{combine }\DIFdelend \DIFaddbegin \DIFadd{combined }\DIFaddend with the Glauber model  
    ...
    where $s$ is the distance from the center of the 
      ...
      nuclear radius.
    \DIFaddbegin \DIFadd{Here WS refers to the Wood-Saxon distribution. 
    }\DIFaddend In Eq.~\ref{eq:TaNuc} the depth of the nucleus is integrated out leaving
    just the transverse \DIFdelbegin \DIFdel{dimension in $T_{A}$}\DIFdelend \DIFaddbegin \DIFadd{dimensions in $T_{A}(\vec{r})$}\DIFaddend .
    The average number of nucleons in the overlap region\DIFaddbegin \DIFadd{, $T_{AA}$, }\DIFaddend is given by
      a convolution of $T_A$ from each of the two nuclei\DIFdelbegin \DIFdel{to produce 
    ...
    \DIFdelbegin \DIFdel{It is $T_{AA}$ that modulates the photon flux. 
...
%DIFDELCMD <     %%%
\DIFdelend The Poisson distribution gives the probability that no collisions 
      occur\DIFdelbegin \DIFdel{at a given $b$ using the mean number of nucleons in the overlap 
      region given by $T_{AA}$:
    }\DIFdelend \DIFaddbegin \DIFadd{:
    }\DIFaddend \begin{equation}\DIFdelbegin \DIFdel{ \label{eg:poisNoCol}
      }\DIFdelend \DIFaddbegin \DIFadd{ \label{eq:poisNoCol}
      }\DIFaddend P_{0}(b)=exp[-T_{AA}(b)\sigma_{NN}],
    \end{equation}
    ...
    \DIFaddbegin \DIFadd{$T_{AA}$ is the mean of the Poisson distribution in Eq.~\ref{eq:poisNoCol}.
    }\DIFaddend The photon flux is averaged over impact parameter by integration of the 
    ...
    \DIFaddbegin \DIFadd{$T_{AA}$ reduces Eq.~\ref{eq:photonFluxFinaltmp} at values of $b$ where the
    ...
    A power-law fit to the \DIFdelbegin \DIFdel{proton photoproduction data }\DIFdelend \DIFaddbegin \DIFadd{photoproduction data off of proton targets }\DIFaddend gives an 
      analytic expression for the energy dependence of the \DIFdelbegin \DIFdel{proton }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend photoproduction 
      cross section.
    The \DIFdelbegin \DIFdel{fitting function depends on the photon-proton center of mass energy. 
    The parameterization of the forward proton }\DIFdelend \DIFaddbegin \DIFadd{forward }\DIFaddend photoproduction cross section \DIFdelbegin \DIFdel{fit:
    }\DIFdelend \DIFaddbegin \DIFadd{is parameterized by
    }\DIFaddend \begin{equation} \label{eq:proElFit}
    ...
    The remaining variables in Eq.~\ref{eq:proElFit} are power-law fit 
      parameters.  
    The $XW^{\epsilon}$ term characterizes Pomeron\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Donnachie:2002en}
}%DIFAUXCMD
}\DIFaddend mediated interactions, and the $YW^{\eta}$ term characterizes meson 
      mediated interactions~\cite{vmd1999}.
    \DIFaddbegin \DIFadd{The Pomeron can be considered to be a pair of gluons in a color singlet 
      state.
    }\DIFaddend \JPsi{}'s high mass relative to the $\pi$ and $\rho^{0}$ renders the second 
    ...
    \DIFdelbegin \DIFdel{Due to this consideration}\DIFdelend \DIFaddbegin \DIFadd{For this reason}\DIFaddend , vector meson dominance asserts that only the 
    ...
    The combination of the mean number of nucleons in the overlapping region
      ...
      \DIFdelbegin \DIFdel{the Glauber 
      model}\DIFdelend \DIFaddbegin \DIFadd{Eq.~\ref{eq:glauber}}\DIFaddend . 
  ...
  \section{The Adeluyi and Bertulani model \label{sec:pQCDapp}}
    ...
    The probability for the photon to fluctuate to a $c\bar{c}$ pair
      depends on \DIFdelbegin \DIFdel{the }\DIFdelend $M_{J/\psi}$, the mass of \JPsi{}, $\Gamma_{l^{+}l^{-}}$, 
    ...
    \end{equation} 
      where \DIFdelbegin \DIFdel{$t_{min}=(M_{J/\psi}^{2}/4k\gamma_{L})^2$}\DIFdelend \DIFaddbegin \DIFadd{$t_{min}=(M_{J/\psi}^{2}/4k\gamma)^2$}\DIFaddend , which is the minimum 
  ...
  \section{\label{sec:ltaTheory}The Leading Twist Approach model}
    ...
    \end{equation}
    where $x=\frac{\omega b}{\gamma_{L}}$, and \DIFdelbegin \DIFdel{$K_{0}^{2}(x)$ }\DIFdelend \DIFaddbegin \DIFadd{$K_{0}(x)$ }\DIFaddend term contributes 
    ...
    The \DIFdelbegin \DIFdel{terms }\DIFdelend \DIFaddbegin \DIFadd{term }\DIFaddend $P_{C}^{i}$ \DIFdelbegin \DIFdel{and $K_{0}$ provide }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend additional ways to distinguish UPC
      events from nuclear collisions experimentally but \DIFdelbegin \DIFdel{leave }\DIFdelend \DIFaddbegin \DIFadd{leaves }\DIFaddend the underlying 
      interaction mechanism the same. 
    For example, \DIFdelbegin \DIFdel{the additional terms }\DIFdelend \DIFaddbegin \DIFadd{this additional term }\DIFaddend in the LTA formulation of the photon flux
      \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{produces }\DIFaddend calculations of asymmetric neutron emission, which separate UPC 
      events from nuclear collisions.
    \DIFaddbegin \DIFadd{In UPC events the number of forward neutrons on either side of the 
  ...
  \subsection{ UPC measurements at RHIC }
    ...
    The fraction of nuclear interaction in all events with neutrons on both sides
      ...
      \cite{emPCite4} \DIFaddbegin \DIFadd{and Section~\ref{sec:nucBreakUp}}\DIFaddend .
    ...
    \DIFaddbegin \DIFadd{These scintillators covered the pseudo-rapidity region 3.05 < $|\eta|$ < 
      3.85.
    }\DIFaddend Events with hits on both sides of the interaction point were categorized as
    ...
    \begin{figure}[!Hhbt]
      ...
      \caption{The energy asymmetry in the ZDCs for \DIFdelbeginFL \DIFdelFL{photon-induced interaction}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{electromagnetic interactions}\DIFaddendFL , 
    ...
    The \DIFdelbegin \DIFdel{$\rho$ }\DIFdelend \DIFaddbegin \DIFadd{$\rho^{0}$ }\DIFaddend measurements by STAR were in good agreement with the \DIFdelbegin \DIFdel{theory
      calculation method }\DIFdelend \DIFaddbegin \DIFadd{STARlight
      calculation }\DIFaddend described in Section~\ref{sec:vdmTheory}.
    The \DIFdelbegin %DIFDELCMD < \JPsi{ } %%%
\DIFdelend \DIFaddbegin \JPsi{} \DIFaddend measurement where limited by statistic as only 10 \JPsi{}
  ...
  \subsection{ UPC \JPsi{} at the LHC}
    ...
    \DIFdelbegin \DIFdel{All major heavy ion experiments, }\DIFdelend ALICE and CMS \DIFdelbegin \DIFdel{, }\DIFdelend have studied the production of UPC events. 
  ...
  At $y$ = 3  for PbPb collisions at the LHC, the cross section has a 
    ...
    10^{-2}$\DIFaddbegin \DIFadd{, where $x$ is the fraction nucleon's momentum carried by the 
    struck parton}\DIFaddend .
  ...
  At $y$ = 0\DIFaddbegin \DIFadd{, }\DIFaddend $x$ $\sim$ 10$^{-3}$, which is a smaller $x$ than at forward 
  ...
  \begin{table} 
   ...
   \DIFaddbeginFL \DIFaddFL{obtained by }\DIFaddendFL the LTA, STARlight, AB methods \DIFaddbeginFL \DIFaddFL{for PbPb collisions at $\sqrt{s_{NN}}$ = 2.76 TeV}\DIFaddendFL . 
...
\DIFdelbegin \DIFdel{The clear discrepancy between the models in Table~\ref{tab:allXsec} 
  ...
  \begin{figure}[h] 
    ...
    \end{center}
    \caption{ \label{fig:ltaAndPqcdNucSub} \DIFdelbeginFL \DIFdelFL{Nuclear }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Both panels show the nuclear 
      }\DIFaddendFL supression factor, $S$, \DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{obtain from the ALICE data points and calculations
      by }\DIFaddendFL the AB \DIFaddbeginFL \DIFaddFL{(top) }\DIFaddendFL and LTA \DIFdelbeginFL \DIFdelFL{methods}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(bottom) models}\DIFaddendFL ~\cite{lta2013.05}. \DIFaddbeginFL \DIFaddFL{$S$ is denoted 
      as $R_{pb}$ in this figure.}\DIFaddendFL }
  \end{figure}
  \begin{figure}
    \begin{center}
    ...
    \caption{ \label{fig:ltaAndPqcdNucSubSTARlight} Nuclear supression factor, $S$, in STARlight method~\cite{lta2013.05}.
      \DIFaddbeginFL \DIFaddFL{The y-axis is labeled $S$ because for the STARlight model $S$ is not 
      necessary equal to $R$.}\DIFaddendFL }
  \end{figure}
