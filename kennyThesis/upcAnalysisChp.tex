...
%%%%%Fix this paragraph%%%%%%
  ...
  \section{\label{sec:TrigDev} Trigger development} 
    ...
    \subsection{\label{sec:l1Trigger} L1 trigger}
      As described in Section~\ref{sec:detTrigg}, the L1 trigger only makes use
        of the calorimeters, and muon systems.
      The fireware embedded in the hardware of these sub detectors is used to 
        make the L1 trigger decisions.
      In this way, the L1 triggers determine when to read out the tracker for 
        for further inspection all while keep the detectors ready every 25 ns.
...
%%%%%%Fix this table%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \subsection{HLT trigger}
      The HLT trigger system is a set of software paths that are run during 
        data taking to decide which events will be permanently recorded (see 
        Section\ref{detTrigg}).
      Every event selected by the L1 trigger system is run through a collection
        of software modules.
      Each collection of modules designed to pick out a particular sample of 
        data is called a path and functions as a trigger in HLT trigger menu.
