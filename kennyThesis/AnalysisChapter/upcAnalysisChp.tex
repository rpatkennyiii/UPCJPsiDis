\chapter{Analysis}	
  ...
  \section{\label{sec:DataSetEvSel} Data Sets and Event Selection}
    \subsection{Data Set}
      ...
      A minimum bias sample was recorded for the sake of estimating 
        efficiencies.
  ...
  \section{\label{sec:sigEx} Signal extraction}
    ...
    The invariant mass distribution for opposite sign dimuons is shown in 
      Fig.~\ref{fig:massFit}. 
    A J/$\psi$ signal is clearly visible together with tails at higher and in
    ...
    The same candidates from Fig.~\ref{fig:massFit} are plotted as a function
    ...
    The information from the invariant mass and $p_{T}$ distributions was 
    ...
    In addition, the J/$\psi$ yield from the mass fit is forced to equal the
      contribution from from the incoherent and coherent process in the 
      fit to the $p_{T}$ distribution. 
    In this way, the correlation between the yield parameters was broken, and 
      the contribution from the three process are left independent of each 
      other.
    ...
    By simultaneously fitting both the mass and $p_{T}$ spectra, the yields
    ...
    The ambiguity between the coherent and incoherent processes in the mass fit
      the ambiguity between the coherent and the photon-photon process was over
      come by combining the two fits.
    Fig.~\ref{fig:simGaussCor} shows the confidence contours for nCo and nGamma
      from the simultaneous fit in Fig.~\ref{fig:simFitMassPtGauss}.  
  ...
  \section{\label{sec:sysCheck} Systematic checks}
    ...
    \begin{table}[!Hhtb]
      \begin{center}
        ...
        \begin{tabular}{|c|c|c|}
          ...
          Neutron tagging  & 5.7\%  \\ \hline
          \JPsi mass & 3.7\%    \\ \hline
          Coherent fraction extraction & 2.9\%    \\ \hline
          ZDC efficiency estimation & 2.2\%    \\ \hline
          \JPsi extraction & 1.9\%    \\ \hline
          MC corrections & 1.25\%    \\ \hline
          Exclusivity cuts & 1 \%    \\ 
          \hline \hline
    ...
    \subsection{ZDC efficiency}
      ...
      The systematic uncertainty due to the uncertainty in the 
        underlaying distribution was estimated by calculating the standard 
        deviation of the least extreme values from Table~\ref{tab:zdcEfficiencySys}.
