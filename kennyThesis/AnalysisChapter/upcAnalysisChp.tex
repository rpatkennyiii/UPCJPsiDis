\chapter{Analysis}
  In this chapter the various parts of the measurements done for this
  ...
  In Section~\ref{sec:mcSim}, the simulations used to estimate the detectors 
  ...
  \section{\label{sec:mcSim} MC simulation}
    ...
    In this way, the theoretical distributions are merged with the realistic 
      response of the detector. 
    The collective combination of the many sub detector responses with the 
    ...
    This class of MC samples corresponds to the theoretical calculations 
      described in Section~\ref{sec:vdmTheory}.
    ...
    Coherent J/$\psi$ production, where the photon couples to the nucleus as
      ...
      nucleon within the nucleus, and the photon-photon process, where the 
      photons from the two nuclei interact with each other to produce a lepton 
      pair directly.
    All three STARlight samples produce a $\mu^{+}$ and $\mu^{-}$ in the final 
    ...
    Because STARlight is not integrated into the standard CMS software 
      framework (CMSSW), a simulation software chain using STARlight was 
    ...
    The three remaining steps take place within the framework. 
...
%%%%%Fix this paragraph%%%%%%
  ...
  \section{\label{sec:TrigDev} Trigger development} 
    ...
    The 2010 rates were used to extrapolate to the interaction rate of the 2011 i
      run. 
    ...
    \subsection{\label{sec:l1Trigger} L1 trigger}
      ...
      Additional triggers that vetoed on energy in HF were commissioned in case
        radiation damage during the run reduced the sensitivity of BSCs.
    ...
    \subsection{HLT trigger}
      ...
      To meet the bandwidth requirements and collect a significant sample
        ...
        were set. 
      The ZDC trigger that was passed through from the L1 was given a additional 
        prescale factor of 11 on the HLT.
  ...
  \section{\label{sec:DataSetEvSel} Event selection}
    ...
    \subsection{Data sets}
      ...
      \begin{table}
          ...
          Physics & 3.46K & 143.3 $\mu$$b^{-1}$ \\ \hline
    ...
    \subsection{Event selection cuts}
      ...
      \begin{table}
        ...
        \begin{tabular}{|c|c|c|} \hline 
          ...
          fake muon rejection & muon quality requirement & 1956\\ \hline
          \multirow{2}{*}{kinematic cut} & J/$\psi$ mass requirement & 662 \\ \hhline{~--}
          & muon detectability cuts & 541 \\ \hline
        \end{tabular}
