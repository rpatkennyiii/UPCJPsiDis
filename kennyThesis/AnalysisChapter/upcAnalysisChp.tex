...
%         \end{array} $
  ...
  \section{\label{sec:sysCheck} Systematic checks}
    \subsection{HF noise threshold}
      The way in which the HF noise distribution is measured effects the events 
        selection.
      In Table~\ref{tab:evSelCutNumbers} the importance of cutting on HF noise
        is evident. 
      The HF cut requires signal consistent with noise and in turn rejects 
        hadronic events. 
      The HF noise cut rejects \textcolor{red}{X} of the events that remaining. 
      ...
      Four different approaches were employed to estimate the systematic effect
        arising from pick a particular method. 
