\chapter{\label{ch:analysis} Analysis}
  ...
  \section{\label{sec:mcSim} \DIFdelbegin \DIFdel{MC simulation}\DIFdelend \DIFaddbegin \DIFadd{Physics generators and Monte Carlo simulations}\DIFaddend }
    ...
    For this reason, the numerical technique of Monte Carlo (MC) simulation is
      often the most effective approach \DIFaddbegin \DIFadd{for describing detector effects}\DIFaddend .
    ...
    As the particles pass through the detector, random numbers are \DIFdelbegin \DIFdel{again }\DIFdelend used
    ...
    The \DIFaddbegin \DIFadd{final goal of the MC simulation is to produce a }\DIFaddend set of events that 
      \DIFdelbegin \DIFdel{are produced resemble as closely as possible the 
      results that would be seen were the physical process to be measured by the detector}\DIFdelend \DIFaddbegin \DIFadd{accurately reproduce what would be measured if the theoretical input 
      describes nature well}\DIFaddend . 

  \DIFaddbegin \subsection{\DIFadd{STARlight and particle gun MC in CMS}}
    \DIFaddend In this thesis, two \DIFdelbegin \DIFdel{main classes of MC simulation }\DIFdelend \DIFaddbegin \DIFadd{classes of generator input }\DIFaddend samples were used, 
    ...
    All three STARlight samples contain a $\mu^{+}$ and $\mu^{-}$ in the final 
      state.
    The second class \DIFdelbegin \DIFdel{uses PYTHIA6 to decay }\DIFdelend \DIFaddbegin \DIFadd{simulates }\DIFaddend \JPsi{} \DIFdelbegin \DIFdel{s produced }\DIFdelend \DIFaddbegin \DIFadd{particles }\DIFaddend with a user 
      defined \pt{} and rapidity distribution \DIFdelbegin \DIFdel{into muon pairs
      }\DIFdelend \DIFaddbegin \DIFadd{and isotropic decay to muon pairs
      in the }\JPsi{} \DIFadd{rest frame}\DIFaddend .
    ...
    \JPsi{} mesons were created according to user defined \pt{} and rapidity
      distributions. 
    \DIFdelbegin \DIFdel{PYTHIA6 \mbox{%DIFAUXCMD
\cite{pythia}
}%DIFAUXCMD
decays the }\DIFdelend \DIFaddbegin \DIFadd{The decay of }\DIFaddend \JPsi{}s to $\mu^{+}$ and $\mu^{-}$ \DIFaddbegin \DIFadd{was simulated with a 
      a uniform decay distribution, corresponding to unpolarized }\JPsi{} 
      \DIFadd{particles}\DIFaddend .
    ...
    This imposes an \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{effective }\DIFaddend momentum threshold on which muons can fire the 
    ...
    In Fig.~\ref{fig:starlightRapPtDist} the \pt{} of \JPsi{}s from the 
      \DIFdelbegin \DIFdel{coherent, }\DIFdelend \DIFaddbegin \DIFadd{STARlight generated coherent, and }\DIFaddend incoherent, and \DIFaddbegin \DIFadd{the dimuon }\pt \DIFadd{for 
      }\DIFaddend photon-photon samples are compared.
...
\DIFdelend \begin{figure}[!Hhbt]
  ...
  \section{\label{sec:TrigDev} Trigger development}
    \DIFaddbegin \DIFadd{A collection of triggers was needed in order to select UPC events from the
      collisions at the LHC. 
    }\DIFaddend The increase in collision rate of the LHC PbPb beams from 2010 to 2011 was
    ...
    To ensure that UPC physics could be explored all while respecting the
      goals of the CMS Heavy Ions group as a whole, a \DIFdelbegin \DIFdel{collection of }\DIFdelend UPC triggers were 
      commissioned. 

    The UPC trigger \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{rate estimates for commissioning the }\DIFaddend 2011 \DIFdelbegin \DIFdel{were created }\DIFdelend \DIFaddbegin \DIFadd{triggers were 
      calculated }\DIFaddend by combining existing triggers from the 2010 run. 
    ...
    \subsection{\label{sec:l1Trigger} L1 trigger}
      The \DIFdelbegin \DIFdel{goal of the }\DIFdelend UPC L1 triggers were \DIFaddbegin \DIFadd{designed }\DIFaddend to record enough data to measure 
      ...
      To achieve this, the loosest muon and electron triggers where paired with
        a trigger on energy in the ZDC \DIFdelbegin \DIFdel{and a veto on }\DIFdelend \DIFaddbegin \DIFadd{were used as a trigger selection 
        criteria, and }\DIFaddend energy in the BSCs \DIFaddbegin \DIFadd{was used as a rejection criteria}\DIFaddend .
      Additional triggers \DIFdelbegin \DIFdel{that vetoed }\DIFdelend \DIFaddbegin \DIFadd{with rejection criteria based }\DIFaddend on energy in HF were 
    ...
    \subsection{HLT trigger}
      \DIFaddbegin \DIFadd{An event must pass the selection criteria of an HLT path in order to be
        recorded. 
      }\DIFaddend As opposed to the L1 trigger, which has access only to information from
        calorimeters and muon chambers, the HLT has access to all \DIFdelbegin \DIFdel{the 
        }\DIFdelend \DIFaddbegin \DIFadd{of the CMS 
        }\DIFaddend sub-detectors including the tracker. 
  ...
  \section{\label{sec:DataSetEvSel} Event selection}
    \DIFdelbegin \DIFdel{In order }\DIFdelend \DIFaddbegin \DIFadd{The unprecedented amounts of data produced by the LHC has made it possible 
      }\DIFaddend to investigate novel physics processes like UPC \JPsi{} production\DIFdelbegin \DIFdel{, the LHC has delivered unprecedented amounts of data}\DIFdelend .
    The data for this analysis \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend recorded during the 2011 LHC PbPb run. 
    During this period, 150 $\mu$$b^{-1}$ \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend recorded by the CMS detector,
      corresponding to over a billion PbPb collisions. 
    Of this, 143 $\mu$$b^{-1}$ \DIFaddbegin \DIFadd{of data }\DIFaddend were used in this analysis.

    \subsection{Data sets}
      \DIFdelbegin \DIFdel{Three }\DIFdelend \DIFaddbegin \DIFadd{The data were dived into three }\DIFaddend specially selected samples\DIFdelbegin \DIFdel{were used for this analysis}\DIFdelend , Physics, Monitoring, 
        and Zero bias\DIFaddbegin \DIFadd{, based on the triggers which recored the events }\DIFaddend (see 
      ...
      By recording this hierarchy of samples, interesting events are selected 
        ...
        criteria. 
      \DIFaddbegin \DIFadd{The purity, which is a measure of how many signal events relative to
      ...
      }\DIFaddend These samples were recorded using subsets of the HLT triggers found in 
      ...
      \DIFdelbegin \DIFdel{This process }\DIFdelend \DIFaddbegin \DIFadd{Neutron production }\DIFaddend is much more common \DIFaddbegin \DIFadd{process }\DIFaddend than the UPC \JPsi{} 
        production.
      \DIFdelbegin \DIFdel{Neutron production }\DIFdelend \DIFaddbegin \DIFadd{This process }\DIFaddend has cross sections on the order of 100 b compared
        to 10 mb predicted \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend \JPsi{} production. 
      ...
      The integrated luminosity for each of the three samples is calculated
        by recording activity in HF\DIFaddbegin \DIFadd{, the forward hadronic calorimeter in CMS
        }\DIFaddend \cite{cmsLumi}. 
  ...
  \section{\label{sec:breakUpDet} Break up determination}
    ...
    By separating events where the ZDC signal is consistent with 1 neutron 
      ...
      both sides, the \DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{fraction of events which correspond to a given 
      }\DIFaddend break-up \DIFdelbegin \DIFdel{modes can be separated
      }\DIFdelend \DIFaddbegin \DIFadd{mode can be measured }\DIFaddend and compared to theory. 
    ...
    \subsection{ZDC signal reconstruction}
      ...
      \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:zdcTs4OvTs5VTs5} demonstrates the consistence of the 
      ...
      The EM\DIFaddbegin \DIFadd{, electromagnetic, }\DIFaddend section of the calorimeter is more densely 
        packed with quartz fibers and therefore has a higher gain relative to 
        the HAD\DIFaddbegin \DIFadd{, hadronic, }\DIFaddend section. 
    ...
    \subsection{Determination of the one neutron thresholds}
      ...
      If the HAD or EM signals measured from \DIFdelbegin \DIFdel{the in time time slices }\DIFdelend \DIFaddbegin \DIFadd{time slices which match the 
        timing for a collision}\DIFaddend , time slices 4 and 5, are less than 2$\sigma$ 
  ...
  \section{\label{sec:sigEx} Signal extraction}
    After all event selection cuts, the \DIFaddbegin \DIFadd{remaining events contain a combination 
      of }\DIFaddend coherent \JPsi{}, incoherent \JPsi{}, and \DIFaddbegin \DIFadd{dimuons from the 
      }\DIFaddend photon-photon process\DIFdelbegin \DIFdel{all contribute to the remaining events}\DIFdelend .
    ...
    In \DIFdelbegin %DIFDELCMD < \pt{} %%%
\DIFdelend \DIFaddbegin \DIFadd{dimuon transverse momentum distribution of }\DIFaddend the photon-photon and 
    ...
    The shape of the photon-photon and coherent \JPsi{} process are very 
      similar in \DIFdelbegin %DIFDELCMD < \pt{}%%%
\DIFdelend \DIFaddbegin \DIFadd{transverse momentum}\DIFaddend .
  ...
  \section{\label{sec:effDet} Efficiency determination}
    ...
    \subsection{Muon efficiencies}
      ...
      The \DIFaddbegin \DIFadd{"}\DIFaddend tag and probe method\DIFaddbegin \DIFadd{" }\DIFaddend is a data driven approach used to measure the 
      ...
      The efficiency from the fits in each bin are \DIFaddbegin \DIFadd{shown }\DIFaddend in Fig.~\ref{fig:tnpTrigMap}.
      \begin{figure}[!Hhbt]
        \centering
