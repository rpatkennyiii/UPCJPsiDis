\chapter{Analysis}
  In this chapter the various parts of the \DIFdelbegin \DIFdel{measurements done for this
    thesis }\DIFdelend \DIFaddbegin \DIFadd{analysis }\DIFaddend are explained. 
  \DIFdelbegin \DIFdel{The following chapter contains seven sections explain each of these parts: 
    Monte Carlo simulation, trigger development, data sets and event selection,
    break-up determination, signal extraction, efficiency determination,
    and systematic checks. 
  }\DIFdelend In Section~\ref{sec:mcSim}, the simulations used to estimate the \DIFdelbegin \DIFdel{detectors 
    }\DIFdelend \DIFaddbegin \DIFadd{detector's 
    }\DIFaddend ability to measure UPC processes are discussed. 
  Section~\ref{sec:TrigDev} explains the considerations that went into the 
    triggers \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend were developed for \DIFdelbegin \DIFdel{the analyses discussed in this thesis.
  How the final events were selected and how triggers were used to separate the
    data in to data }\DIFdelend \DIFaddbegin \DIFadd{this measurement.
  The event selection for the various data }\DIFaddend sets is detailed in 
  ...
  Extraction of the number of events from each of the three physics processes 
    ...
    \DIFdelbegin \DIFdel{from the final selected events }\DIFdelend is discussed is Section~\ref{sec:sigEx}.
  The determination of the detectors efficiency for measuring UPC events is 
    explained in Section~\ref{sec:effDet}.
  \DIFaddbegin \DIFadd{Finally, }\DIFaddend Section~\ref{sec:sysCheck} lays out \DIFdelbegin \DIFdel{how }\DIFdelend the systematic uncertainties 
    \DIFdelbegin \DIFdel{are 
    estimated}\DIFdelend \DIFaddbegin \DIFadd{estimates on the measurements}\DIFaddend .
  ...
  \section{\label{sec:mcSim} MC simulation}
    ...
    First, particles are generated according \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend theoretical distributions.
    ...
    In this way, the theoretical distributions are \DIFdelbegin \DIFdel{merged with the realistic 
      response }\DIFdelend \DIFaddbegin \DIFadd{convolved with a realistic 
      model }\DIFaddend of the detector\DIFaddbegin \DIFadd{'s response. 
    ...
    The set of events that are produced resemble as closely as possible the 
      ...
      by the detector and physical process being simulated}\DIFaddend .
    \DIFdelbegin \DIFdel{The collective combination of the many sub detector responses with the theoretical distributions emerges from the successive creation of random
      events. 
    The result is the convolved response of the detector with the underlying 
      physical process that is to be studied. 
}\DIFdelend 

    In this thesis, two main classes of MC simulation samples were used\DIFdelbegin \DIFdel{. 
    The first class uses STARlight to generate events.
    This class of MC }\DIFdelend \DIFaddbegin \DIFadd{, 
      STARlight and a particle gun.
    The STARlight }\DIFaddend samples corresponds to the theoretical calculations 
      described in Section~\ref{sec:vdmTheory}\DIFdelbegin \DIFdel{.
    There are }\DIFdelend \DIFaddbegin \DIFadd{, while the particle gun produces
      particles with a user defined momentum distribution. 
    For STARlight }\DIFaddend three different physical process \DIFdelbegin \DIFdel{described.
    Coherent }\DIFdelend \DIFaddbegin \DIFadd{are simulated;
      coherent }\DIFaddend J/$\psi$ production, where the photon couples to the nucleus as
      a whole, incoherent J/$\psi$ production, where the photon couples to a
      nucleon within the nucleus, and \DIFdelbegin \DIFdel{the }\DIFdelend photon-photon \DIFdelbegin \DIFdel{process}\DIFdelend \DIFaddbegin \DIFadd{interactions}\DIFaddend , where the 
      photons from the two nuclei interact with each other to produce a \DIFdelbegin \DIFdel{lepton 
      pair 
      }\DIFdelend \DIFaddbegin \DIFadd{pair 
      of oppositely charge muons }\DIFaddend directly.
    All three STARlight samples \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{contain }\DIFaddend a $\mu^{+}$ and $\mu^{-}$ in the final 
      state\DIFdelbegin \DIFdel{that interacts with the detector}\DIFdelend .
    The second class uses PYTHIA6 to decay J/$\psi$s \DIFdelbegin \DIFdel{with a given
      input }\DIFdelend \DIFaddbegin \DIFadd{produced with a user 
      defined }\DIFaddend $p_{T}$ and rapidity distribution \DIFdelbegin \DIFdel{.
    Two samples of this class of particle gun data were produced each with 
      different $p_{T}$ distributions (See Fig.).
}\DIFdelend \DIFaddbegin \DIFadd{into muon pairs.
}\DIFaddend 

    Because STARlight is not integrated into the standard CMS software 
      framework (CMSSW), a simulation software chain \DIFdelbegin \DIFdel{using STARlight was developedfor the analysis described in this thesis.
    The software chain used for producing the STARlight samples has five steps.
    }\DIFdelend \DIFaddbegin \DIFadd{with 5 steps was developed.
    }\DIFaddend First, STARlight is run in the specified mode, and a single file is 
      created for each physics process\DIFdelbegin \DIFdel{, for this thesis, one file for the 
    ...
    The output from the STARlight generator is in a format specific to 
      STARlight, therefore, the output from the original generation step is then }\DIFdelend \DIFaddbegin \DIFadd{. 
    In step 2 the STARlight output file is }\DIFaddend converted to the Les Houches (LHE) 
      format \DIFdelbegin \DIFdel{. 
    In this conversion to LHE format, either the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lheFormat}
}%DIFAUXCMD
, and the momentum of the }\DIFaddend parent J/$\psi$ \DIFdelbegin \DIFdel{for the 
      J/$\psi$ production samples, }\DIFdelend or the 
      initial photon-photon pair \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend added to the \DIFdelbegin \DIFdel{LHE output file.
    The standard STARlight output only includes }\DIFdelend \DIFaddbegin \DIFadd{record of each event.
    The event record produced by STARlight only contains }\DIFaddend the final state 
      particles.
\DIFdelbegin \DIFdel{Additionally, the initial output from STARlight is split into a collection 
      of smaller LHE files so that each of the smaller samples can be 
      processed in parallel.
    Each of the LHE files is }\DIFdelend \DIFaddbegin 

    \DIFadd{To process the events in parallel, the STARlight files are subdivided 
      into in step 2, creating several LHE files from a single STARlight file.
    The LHE files are }\DIFaddend used as input to CMSSW.
    \DIFdelbegin \DIFdel{The three remaining steps }\DIFdelend \DIFaddbegin \DIFadd{Steps 3 to 5 }\DIFaddend take place within \DIFdelbegin \DIFdel{the framework. 
    First }\DIFdelend \DIFaddbegin \DIFadd{CMSSW. 
    In step three }\DIFaddend the generated particles are propagated through the GEANT4 
      \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{geant}
}%DIFAUXCMD
}\DIFaddend detector simulation.
    This accounts for all the interactions with the detector and produces as 
      ...
      taking.
    \DIFdelbegin \DIFdel{The next two steps are identical to }\DIFdelend \DIFaddbegin \DIFadd{Steps 4 and 5 are processed using the same software as in }\DIFaddend data taking.
    \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In step 4 the }\DIFaddend reconstruction software used during data taking is run on 
      the output of the detector simulation
    \DIFdelbegin \DIFdel{, and last, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend output of the reconstruction is reduced to the information that is 
      needed for the final analysis \DIFaddbegin \DIFadd{in the final step}\DIFaddend .

    The particle gun samples were created entirely within CMSSW.
    \DIFdelbegin \DIFdel{An interface to PYTHIA6 is included within CMSSW, which takes }\DIFdelend J/$\psi$ \DIFaddbegin \DIFadd{mesons were created according to user defined }\DIFaddend $p_{T}$ and rapidity
      distributions\DIFdelbegin \DIFdel{as input. 
    The J/$\psi$ are created according to the input distributions, and then uses
      }\DIFdelend \DIFaddbegin \DIFadd{. 
    }\DIFaddend PYTHIA6 \DIFdelbegin \DIFdel{to decay }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{pythia}
}%DIFAUXCMD
decays }\DIFaddend the J/$\psi$s to $\mu^{+}$ and $\mu^{-}$.
    ...
    The \DIFdelbegin \DIFdel{five MC samples}\DIFdelend \DIFaddbegin \DIFadd{various MC samplesv}\DIFaddend , three STARlight samples \DIFdelbegin \DIFdel{, and two particle gun samples,
      }\DIFdelend differ primarily in the 
      $p_{T}$ distribution \DIFaddbegin \DIFadd{and the polarization }\DIFaddend of the J/$\psi$s produced\DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{. 
    The }\DIFaddend polarization of the \DIFdelbegin \DIFdel{J/$\psi$s, which effects }\DIFdelend \DIFaddbegin \JPsi \DIFadd{effects the }\DIFaddend angle at which the muon daughters
...
%%%%%Fix this paragraph%%%%%%
    ...
    However in the lab frame of the detector, the muon daughters which are 
      ...
      frame \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{oniaPol}
}%DIFAUXCMD
}\DIFaddend .
...
%%%%%Fix this paragraph%%%%%%
  ...
  \section{\label{sec:TrigDev} Trigger development} 
    ...
    The 2010 rates were used to extrapolate to the interaction rate \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{expected 
      prior to }\DIFaddend the 2011 \DIFdelbegin \DIFdel{i
      }\DIFdelend run. 
    ...
    The extrapolated rates allowed for \DIFdelbegin \DIFdel{a package }\DIFdelend \DIFaddbegin \DIFadd{the create of a package of }\DIFaddend triggers to 
    ...
    \subsection{\label{sec:l1Trigger} L1 trigger}
      The goal of the \DIFaddbegin \DIFadd{UPC }\DIFaddend L1 triggers was to record enough data to measure 
        \DIFaddbegin \DIFadd{UPC }\JPsi \DIFadd{production via the }\DIFaddend dimuons and dielectrons \DIFdelbegin \DIFdel{in UPC events}\DIFdelend \DIFaddbegin \DIFadd{channels}\DIFaddend .
      To achieve this, the loosest muon \DIFdelbegin \DIFdel{trigger and lowest threshold ECAL 
        }\DIFdelend \DIFaddbegin \DIFadd{and electron }\DIFaddend triggers where paired with
      ...
      Additional triggers that vetoed on energy in HF were commissioned in case
        radiation damage during the run reduced the sensitivity of \DIFdelbegin \DIFdel{BSCs.
      The L1 package that was constructed for the analysis of UPC J/$\psi$ 
        is presented }\DIFdelend \DIFaddbegin \DIFadd{the BSCs.
      These triggers are summarized }\DIFaddend in Table~\ref{tab:l1Triggers2011}. 
...
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{|l|l|l|l|l|}
          ...
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_MuOpen\_ZdcCalo\_NotBscMinBiasThresh2\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MuonOpen and (ZDC$^{+}$~or~ZDC$^{-}$) and BSC veto }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{2.1 }& \DIFaddFL{1 }& \DIFaddFL{1 }& \DIFaddendFL Physics \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_EG2\_ZdcCalo\_NotBscMinBiasThresh2\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ECAL2 and (ZDC$^{+}$~or~ZDC$^{-}$) and BSC veto }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{1.8 }& \DIFaddFL{2 }& \DIFaddFL{2 }& \DIFaddendFL Physics \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_EG5\_ZdcCalo\_NotBscMinBiasThresh2\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ECAL5 and (ZDC$^{+}$~or~ZDC$^{-}$) and BSC veto }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.3 }& \DIFaddFL{1 }& \DIFaddFL{3 }& \DIFaddendFL Physics \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_ZdcCaloMinus\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(ZDC$^{+}$~or~ZDC$^{-}$) }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{Monitor }%DIFDELCMD < \\  \hline
%DIFDELCMD <           %%%
\DIFdelFL{L1}%DIFDELCMD < \_ZdcCaloMinus\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{35 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{1500 }& \DIFaddFL{4 }& \DIFaddendFL Monitor \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_MuOpen\_ZdcCalo\_NotHcalHfCoincidencePm\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MuonOpen and (ZDC$^{+}$~or~ZDC$^{-}$) and HF veto }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0 }& \DIFaddFL{off }& \DIFaddFL{5 }& \DIFaddendFL Backup \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_EG2\_ZdcCalo\_NotHcalHfCoincidencePm\_BptxAND %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ECAL2 and (ZDC$^{+}$~or~ZDC$^{-}$) and HF veto }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0 }& \DIFaddFL{off }& \DIFaddFL{6 }& \DIFaddendFL Backup \\  \hline
          \DIFdelbeginFL \DIFdelFL{L1}%DIFDELCMD < \_EG5\_ZdcCalo\_NotHcalHfCoincidencePm\_BptxAND %%%
...
\DIFdelendFL \end{tabular}
    ...
    \subsection{HLT trigger}
      As opposed to the L1 trigger, which \DIFdelbegin \DIFdel{reads out the tracker}\DIFdelend \DIFaddbegin \DIFadd{has access only to information from
        calorimeters and muon chambers}\DIFaddend , the HLT has access to \DIFdelbegin \DIFdel{the tracker information}\DIFdelend \DIFaddbegin \DIFadd{all the 
        sub-detectors including the tracker}\DIFaddend . 
...
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{|l|l|l|l|l|l|}
          ...
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuMuPixel%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MuPixel}\DIFaddendFL \_SingleTrack & \DIFaddbeginFL \DIFaddFL{0.52 }& \DIFaddFL{1 }& \DIFaddFL{1 }& \DIFaddFL{1 }& \DIFaddendFL Physics   \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuEG2Pixel%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EG2Pixel}\DIFaddendFL \_SingleTrack & \DIFaddbeginFL \DIFaddFL{1.65 }& \DIFaddFL{2 }& \DIFaddFL{1 }& \DIFaddFL{2 }& \DIFaddendFL Physics   \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuEG5Pixel%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{EG5Pixel}\DIFaddendFL \_SingleTrack & \DIFaddbeginFL \DIFaddFL{0.26 }& \DIFaddFL{1 }& \DIFaddFL{1 }& \DIFaddFL{3 }& \DIFaddendFL Physics   \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIMinBiasZDC\_Calo%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ZDC}\DIFaddendFL \_PlusOrMinus \DIFdelbeginFL %DIFDELCMD < \_v1  %%%
\DIFdelendFL & \DIFaddbeginFL \DIFaddFL{3.6 }& \DIFaddFL{1500 }& \DIFaddFL{11 }& \DIFaddFL{4 }& \DIFaddendFL Monitor  \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIMinBiasZDC%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ZDC}\DIFaddendFL \_PlusOrMinusPixel\_SingleTrack \DIFdelbeginFL %DIFDELCMD < \_v1   %%%
\DIFdelendFL & \DIFaddbeginFL \DIFaddFL{2.8 }& \DIFaddFL{1500 }& \DIFaddFL{1 }& \DIFaddFL{4 }& \DIFaddendFL Monitor \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuHcalHfMuPixel%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{HfMuPixel}\DIFaddendFL \_SingleTrack & \DIFaddbeginFL \DIFaddFL{0 }& \DIFaddFL{off }& \DIFaddFL{off }& \DIFaddFL{5 }& \DIFaddendFL Backup   \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuHcalHfEG2Pixel%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{HfEG2Pixel}\DIFaddendFL \_SingleTrack & \DIFaddbeginFL \DIFaddFL{0 }& \DIFaddFL{off }& \DIFaddFL{off }& \DIFaddFL{6 }& \DIFaddendFL Backup   \\ \hline
          \DIFdelbeginFL \DIFdelFL{HLT}%DIFDELCMD < \_HIUPCNeuHcalHfEG5Pixel%%%
...
\DIFdelendFL \end{tabular}
      ...
      To meet the bandwidth \DIFdelbegin \DIFdel{the }\DIFdelend requirements and collect a significant sample
        of data for estimating efficiencies, the prescales for the triggers 
        were set. 
      \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As an example of the balancing of the prescales, the  }\DIFaddend ZDC trigger that 
        was passed through from the L1 was given a \DIFdelbegin \DIFdel{larger 
        prescale to account for the higher rate relative to the more selective 
        ZDC path , which }\DIFdelend \DIFaddbegin \DIFadd{additional prescale factor 
        of 11 on the HLT.
      The ZDC path that }\DIFaddend also required a pixel track on the HLT\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, which used 
        the same L1 seed, was only prescaled at the L1.
      The prescale of 11 was set to ensure that a large enough sample of the 
        pixel track ZDC triggers overlapped with the ZDC L1 only triggers 
        so that efficiency of the pixel track requirement in the trigger could
        be estimated from the tracks lost. 
      The prescale on the ZDC triggers were set to minimize the over all rate 
        from the ZDC triggers all while ensuring a precision of 1\% from the 
        statistical errors from the efficiency measurement. 
}\DIFaddend 
  ...
  \section{\label{sec:DataSetEvSel} Event selection}
    ...
    Of this, 143 $\mu$$b^{-1}$ \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend used in this analysis.

    \subsection{Data sets}
      Three specially selected samples were used for the present analysis\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, 
        Physics, Monitoring, and Zero bias, }\DIFaddend see Table~\ref{tab:sampleLumiNevt}\DIFdelbegin \DIFdel{).
      }\DIFdelend \DIFaddbegin \DIFadd{.
      By recording this hierarchy of samples, interesting events are selected 
       ...
       ZDC triggered samples allow for the investigation of the selection 
        criteria. 
      }\DIFaddend These samples were recorded using subsets of the \DIFaddbegin \DIFadd{HLT }\DIFaddend triggers found in 
        \DIFaddbegin \DIFadd{Table~\ref{tab:hltTriggers2011} of }\DIFaddend Section~\ref{sec:TrigDev}.
      ...
      A ZDC triggered \DIFaddbegin \DIFadd{monitoring }\DIFaddend sample was recorded for the sake of estimating
        efficiencies.
      \DIFdelbegin \DIFdel{Last}\DIFdelend \DIFaddbegin \DIFadd{Lastly}\DIFaddend , a zero bias sample was recored for investigating the ZDC and the 
        noise distributions of HF.
  \DIFdelbegin \DIFdel{By recording this hierarchy of samples, interesting events are selected 
        with a much higher purity in the physics sample, while the zero bias and 
      ZDC triggered samples allow for the investigation of the selection 
        criteria. 
  }\DIFdelend 
      ...
      To record the physics sample containing the J/$\psi$ signal, a muon trigger
        ...
        be energy in at least one of \DIFdelbegin \DIFdel{two sides of the ZDC}\DIFdelend \DIFaddbegin \DIFadd{the two ZDCs}\DIFaddend . 
    ...
    \subsection{Event selection cuts}
      ...
      \begin{table}
        ...
        \begin{tabular}{|c|c|c|} \hline 
          \DIFdelbeginFL \DIFdelFL{cut }%DIFDELCMD < & %%%
\DIFdelFL{cut }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Cut }\DIFaddendFL type & \DIFdelbeginFL \DIFdelFL{events }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Cut }& \DIFaddFL{Events }\DIFaddendFL \\ \hline
          \DIFaddbeginFL \DIFaddFL{-- }& \DIFaddendFL all triggered & \DIFdelbeginFL \DIFdelFL{-- }%DIFDELCMD < & %%%
\DIFdelendFL 346841 \\ \hline
          \DIFaddbeginFL \multirow{3}{*}{beam background rejection} & \DIFaddendFL good vertex requirement & \DIFdelbeginFL \DIFdelFL{beam background rejection }%DIFDELCMD < & %%%
\DIFdelendFL 340997 \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \hhline{~--}
          & \DIFaddendFL beam halo muon rejection & \DIFdelbeginFL \DIFdelFL{beam background rejection }%DIFDELCMD < & %%%
\DIFdelendFL 302777 \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \hhline{~--}
          & \DIFaddendFL cluster shape compatibility requirement & \DIFdelbeginFL \DIFdelFL{beam background rejection }%DIFDELCMD < & %%%
\DIFdelendFL 233590 \\ \hline
          \DIFaddbeginFL \multirow{3}{*}{hadronic interaction rejection} & \DIFaddendFL single-sided neutron requirement & \DIFdelbeginFL \DIFdelFL{hadronic interaction rejection }%DIFDELCMD < & %%%
\DIFdelendFL 149992 \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \hhline{~--}
          & \DIFaddendFL two track requirement & \DIFdelbeginFL \DIFdelFL{hadronic interaction rejection }%DIFDELCMD < & %%%
\DIFdelendFL 32732 \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \hhline{~--}
          & \DIFaddendFL HF signal rejection & \DIFdelbeginFL \DIFdelFL{hadronic interaction rejection }%DIFDELCMD < & %%%
\DIFdelendFL 5392 \\ \hline
          \DIFdelbeginFL \DIFdelFL{muon quality requirement }%DIFDELCMD < & %%%
\DIFdelendFL fake muon rejection & \DIFdelbeginFL \DIFdelFL{1956}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{muon quality requirement }& \DIFaddFL{2047 }\DIFaddendFL \\ \hline
          \DIFaddbeginFL \multirow{2}{*}{kinematic cut} & \DIFaddendFL J/$\psi$ mass requirement & \DIFdelbeginFL \DIFdelFL{kinematic cut }%DIFDELCMD < & %%%
\DIFdelFL{662 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{696 }\DIFaddendFL \\ \DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \hhline{~--}
          & \DIFaddendFL muon detectability cuts & \DIFdelbeginFL \DIFdelFL{kinematic cut }%DIFDELCMD < & %%%
  ...
  \section{\label{sec:effDet} Efficiency determination}
    \subsection{Muon efficiencies}
      ...
      \begin{figure}[!Hh]
        ...
        \caption{Fits to tag and probe pairs in the J/$\psi$ mass region \DIFaddbeginFL \DIFaddFL{for
          pairs with a probe 2 < |$\eta$| < 2.2 and 1.55 < }\pt \DIFaddFL{< 1.8 GeV}\DIFaddendFL .}
