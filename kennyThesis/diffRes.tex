\chapter{Results and summary}
  ...
  %
    ...
    Previous measurements carried by the ALICE Collaboration measured the cross
      section without any \DIFdelbegin \DIFdel{break-up requirement}\DIFdelend \DIFaddbegin \DIFadd{neutron emmission requirment}\DIFaddend . 
    ...
    Figure~\ref{fig:coJpXsec} shows the measured cross section compared to the 
      ALICE data points, as well as \DIFdelbegin \DIFdel{STARligh}\DIFdelend \DIFaddbegin \DIFadd{STARlight}\DIFaddend , AB and LTA models 
    ...
    \begin{figure*}[!Hhtb]
      \begin{center}
        ...
        \caption{\label{fig:rNeutDimuCorr}Rapidity distribution of \JPsi{} in the
          ...
          for the \DIFdelbeginFL \DIFdelFL{low-}\DIFdelendFL \pt{} \DIFaddbeginFL \DIFaddFL{< 0.15 GeV }\DIFaddendFL \JPsi{} (left), \DIFdelbeginFL \DIFdelFL{high-}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.15 < }\DIFaddendFL \pt{} \DIFaddbeginFL \DIFaddFL{< 1.0 GeV }\DIFaddendFL \JPsi{} (right) and dimuons from
  ...
  \DIFaddbegin \section{\label{sec:summary}\DIFadd{Summary}}
    \DIFadd{As physicists' understanding of the QGP has deepened over the past 30 years
    ...
    This thesis contributes to the understanding of the initial state through
      the measurement of the UPC }\JPsi{} \DIFadd{photoproduction cross section. 
  }

    \DIFadd{Ultra-peripheral collisions are clean probe of the initial state. 
    In UPC }\JPsi{} \DIFadd{photoproduction, the nuclei interact through the 
      electromagnetic force precluding the possibility of creating a collective
      medium.
    The theoretical models of coherent UPC }\JPsi{} \DIFadd{photoproduction model these 
    ...
    The Weis}\"{a}\DIFadd{cker-Williams approximation \mbox{%DIFAUXCMD
\cite{WWFermi}
}%DIFAUXCMD
is used to calculate 
    ...
    The interaction of these photons with the nucleus is either calculated 
      ...
      \mbox{%DIFAUXCMD
\cite{pQCD2013.02, lta2012.03}
}%DIFAUXCMD
, or by using the Glauber model approach 
...
. 
    ...
    \DIFadd{The analysis in this thesis consists of three major components, development
    ...
    The trigger development involved designing a trigger based on rate estimates
      ...
      event selection.
    The number of }\JPsi{} \DIFadd{candidates was measured by first applying a set 
    ...
    The cross section was calculated by combining the efficiency with the 
      measured luminosity and number of coherent }\JPsi{}\DIFadd{.
    ...
    \DIFadd{The UPC }\JPsi{} \DIFadd{photoproduction cross section in the Xn0n break-up mode, $\frac{d\sigma^{J/\psi}_{co}}{dy}$ (Xn0n),
    ...
    When rescaled by a factor of 5.06 to account for the difference of break-up mode between 
      the measurement in this thesis and the ALICE result in~\mbox{%DIFAUXCMD
\cite{Abelev:2012ba,Abbas:2013oua}
}%DIFAUXCMD
, the 
      result of 1.80 $\pm$ 0.37 (stat) $^{+0.22}_{-0.25}$ (syst) mb was found to be consistent with the 
      predictions in \mbox{%DIFAUXCMD
\cite{pQCD2013.02}
}%DIFAUXCMD
.  
    The calculation in \mbox{%DIFAUXCMD
\cite{pQCD2013.02}
}%DIFAUXCMD
is also favored by the ALICE measurements. 
    Of the gluon distributions used in \mbox{%DIFAUXCMD
\cite{pQCD2013.02}
}%DIFAUXCMD
, a gluon distribution with 
      moderately strong gluon shadowing, EPS09~\mbox{%DIFAUXCMD
\cite{Eskola:2009uj}
}%DIFAUXCMD
, is consistent with both the results from this
      thesis and the previous ALICE results. 
    This indicates that at the scale of the mass of the }\JPsi{} \DIFadd{the nucleus gluon density is 
    ...
    At this scale the nucleus can not be represented as a collection of nucleons as in the 
      Glauber like model described in \mbox{%DIFAUXCMD
\cite{vmd1999}
}%DIFAUXCMD
.
    ...
    \DIFadd{The measurements in this thesis confirm the ability to increase the 
...
\fi
 ...
 \DIFdelbegin \section{%DIFDELCMD < \label{sec:summary}%%%
\DIFdel{Summary}}
    %DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{As physicists' understanding of the QGP has deepened over the past 30 years
      of doing experimental heavy ion physics, the questions surrounding the
      QGP have shift from the confirmation of creation of a deconfined state
      to understanding the properties of the state that is created.
    It appears that the QGP is a hot nearly viscosity free fluid of strongly 
    ...
    UPC events can provide this needed knowledge. 
    This thesis contributes to the understanding of the initial state through
      the measurement of the UPC }%DIFDELCMD < \JPsi{} %%%
\DIFdel{photoproduction cross section. 
  }%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{Ultra-peripheral collisions are clean probe of the initial state. 
    In UPC }%DIFDELCMD < \JPsi{} %%%
\DIFdel{photoproduction, the nuclei interact through the 
      electromagnetic force precluding the possibility of creating a collective
      medium.
    The theoretical models of coherent UPC }%DIFDELCMD < \JPsi{} %%%
\DIFdel{photoproduction model these 
      electromagnetic interactions by combining a semi-classical calculation 
      of the photon flux with a variety of phenomenological and QCD based 
      calculations of the nuclear gluon density. 
    The Weis}%DIFDELCMD < \"{a}%%%
\DIFdel{cker-Williams approximation \mbox{%DIFAUXCMD
\cite{WWFermi}
}%DIFAUXCMD
is used to calculate 
      the flux of photons that surround the colliding nuclei. 
...
%DIFDELCMD <     %%%
\DIFdel{The analysis in this thesis consists of three major components, development
      of a trigger, estimation of efficiency, and measurement of signal events.
    The trigger development involved designing a trigger based on rate estimates
      from past data that ensured a sample that could be used for both measuring
      the signal and estimate the efficiency of the trigger, reconstruction, and
      event selection.
    The number of }%DIFDELCMD < \JPsi{} %%%
\DIFdel{candidates was measured by first applying a set 
      of event selection cuts that rejected background events such as hadronic
      collisions and beam gas collisions, then fitting the remaining events to
      templates from simulation to separate the three remaining physics processes,
    ...
    The acceptance and reconstruction efficiency were estimated from MC.
    The cross section was calculated by combining the efficiency with the 
      measured luminosity and number of coherent }%DIFDELCMD < \JPsi{}%%%
\DIFdel{.
    The statistical uncertainties were taken from the template fit.
    The systematic uncertainties were estimated by varying the method used on 
      each component of the analysis. 
...
%DIFDELCMD <     %%%
\DIFdel{The UPC }%DIFDELCMD < \JPsi{} %%%
\DIFdel{photoproduction cross section in the Xn0n break-up mode, $\frac{d\sigma^{J/\psi}_{co}}{dy}$ (Xn0n),
      was found to be 356 $\pm$ 71 (stat) $^{+43}_{-50}$ (syst) $\mu$b. 
    When rescaled by a factor of 5.06 to account for the difference of break-up mode between 
      the measurement in this thesis and the ALICE result in~\mbox{%DIFAUXCMD
\cite{Abelev:2012ba,Abbas:2013oua}
...
, is consistent with both the results from this
      thesis and the previous ALICE results. 
    This indicates that at the scale of the mass of the }%DIFDELCMD < \JPsi{} %%%
\DIFdel{the nucleus gluon density is 
      significantly suppressed compared to the gluon densities of a nucleon.
    At this scale the nucleus can not be represented as a collection of nucleons as in the 
      Glauber like model described in \mbox{%DIFAUXCMD
\cite{vmd1999}
...
.
    In addition, the }%DIFDELCMD < \JPsi{}%%%
\DIFdel{-neutron rapidity correlation shown in Fig.~\ref{fig:r2} by 
      by the $R_{opp/same}$ ratio demonstrates the potential to separate the low-$x$ 
      and high-$x$ contributions to the photoproduction cross section.
  }%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{The measurement in this thesis confirms the ability to increase the knowledge of the 
